...
class VariantAxes:
    ...
    model_openai: Dict[str, Any] = None
...
class ProductHierarchyClassifier:
    ...
    def __init__(self, coarse_level):
        self.coarse_level = coarse_level #fine, #coarse #superfine
    ...
    def extract_variant_axes(self, row: pd.Series) -> VariantAxes:
        ...
        axes.model_openai = self.extract_openai_model(row)
        return axes
    ...
    def extract_brand(self, row: pd.Series) -> str:
        ...
        brand =  row.get("final_brand_with_only_most_common_brands")
        if brand:
            brand = brand.lower().strip()
    ...
    def extract_openai_model(self, row: pd.Series) -> str:
        model_openai = {}
        """Extract and normalize model identifier"""
        # parsed = row.get("parsed", {})
        model = ""
        ...
        model = row.get("model_openai", "")
        if model:
            model_openai['model'] = self.normalize_model(model)
            return model_openai
        ...
        # match = re.search(r"\b([A-Z0-9-]{3,}[A-Z0-9/-]*)\b", text, re.IGNORECASE)
        # if match:
        #     openai_model['model'] =  self.normalize_model(match.group(1))
        #     return openai_model
    ...
    def create_product_groups(self, df: pd.DataFrame) -> None:
        ...
        #fine, #coarse #superfine
        for _, row in df.iterrows():

            if self.coarse_level == 'superfine':
                model = row.get('human_readable_full_name', '')
                brand = ''
                variant = ''
                if model == '':
                    brand = row.get('brand_final', 'other')
                    model = row.get('family_final') if pd.notna(row.get('family_final')) else row.get('model_clean', 'unknown')
                    variant = row.get('parsed_model_name') if pd.notna(row.get('parsed_model_name')) else ''
            elif self.coarse_level == 'fine':
                brand = row.get('brand_final', 'other')
                model = row.get('family_final') if pd.notna(row.get('family_final')) else row.get('model_clean', 'unknown')
                variant = row.get('parsed_model_name') if pd.notna(row.get('parsed_model_name')) else ''
                
            elif self.coarse_level == 'coarse':
                brand = row.get('final_brand_with_only_most_common_brands', 'unknown')
                model = row.get('corrected_family') if pd.notna(row.get('corrected_family')) else row.get('model_clean', 'unknown')
                variant = ""
            ...
            try:
            # Generate group ID
                group_id = self.generate_group_id(brand, model, variant)
            except:
                print(row)
    ...
    # def generate_group_id(self, brand: str, model: str, variant: str) -> str:
    ...
    #     variant = re.sub(r'[^a-z0-9]', '_', variant.lower())
    #     # brand_clean = safe_clean(brand)
    #     # model_clean = safe_clean(model)
    #     if variant== "":
    #         if brand_clean =="":
    #             return f"{model_clean}"
    #         else:
    #             return f"{brand_clean}_{model_clean}"
    #     else:
    #         return f"{brand_clean}_{model_clean}_{variant}"
    import re
    
    def generate_group_id(self, brand: str, model: str, variant: str) -> str:
        """Generate deterministic group ID without redundant overlaps"""
        ...
        brand_clean = safe_clean(brand)
        model_clean = safe_clean(model)
        variant_clean = safe_clean(variant)
    
        # Tokenize into words
        brand_words = set(brand_clean.split("_"))
        model_words = [w for w in model_clean.split("_") if w and w not in brand_words]
        variant_words = [w for w in variant_clean.split("_") 
                         if w and w not in brand_words and w not in model_words]
    
        # Rebuild cleaned strings
        model_clean = "_".join(model_words)
        variant_clean = "_".join(variant_words)
    
        # Construct ID
        if not variant_clean:
            if not brand_clean or brand_clean == "":
                return model_clean or ""
            else:
                return f"{brand_clean}_{model_clean}".strip("_")
        else:
            return f"{brand_clean}_{model_clean}_{variant_clean}".strip("_")
    ...
    def generate_variant_id(self, group_id: str, axes: VariantAxes) -> str:
        ...
        # if axes.model_openai:
            # model_str = self.serialize_axis('model:', axes.model_openai)
            # variant_parts.append(model_str)
...
def main():
    ...
    parser.add_argument('--coarse_level', type=str, help='Process only N samples for testing')
    ...
    df["parsed"] = [parse_openai_json(val, idx) for idx, val in df["normalized"].items()]
    df = df[df['normalized'].notna()]
    df['brand'] = df['brand'].fillna('').astype(str).str.lower().str.strip()
    df['model'] = df['model'].fillna('').astype(str).str.lower().str.strip()


    
    # Initialize classifier
    classifier = ProductHierarchyClassifier(args.coarse_level)
